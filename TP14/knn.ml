...
type arbre = Nil | Noeud of (arbre * float) list
...
let dimensional_tree seq =
	let rec step_i l i n = (*Construit l'arbre*)
		if l = [] then Nil
		else
		let l' = List.map (fun (im, res) -> im.(i mod n)) l in 
		let md = mediane l' in
		let a1 = ref [] in 
		let a2 = ref [] in 
		let rec insert t = 
			...
			|  (im, res) :: t' -> 
				...
				insert t'
		in
		let arbre_g = step_i a1 (i+1) n in 
		let arbre_d = step_i a2 (i+1) n in 
		Noeud(arbre_g, md, arbre_d)
	in 
	let l = List.of_seq seq in 
	let n = Array.length (List.hd l) in 
	step_i l 0 n 
