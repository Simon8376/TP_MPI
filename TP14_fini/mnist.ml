...
let of_channel channel =
	...
	let dim0_size = Array.fold_left (fun acc x -> if acc = -1 then 1 else acc * x) (-1) size in
	{ channel; datatype; dimensions; size ; dim0_size }

let open_in filename =
	of_channel (Stdlib.open_in_bin filename)
...
let get idx pos =
	seek_in idx.channel (item_size Int + item_size Int * idx.dimensions + pos * idx.dim0_size * item_size idx.datatype);
...
let mnist_list idx1 idx2 = (*Construit une séquence de couples formées d'une image et de la réponse où idx1 spnt des images, idx2 des réponses*)
	let rec aux i = fun () ->
		try
			Seq.Cons((get idx1 i, (get idx2 i).(0)), aux (i+1))
		with End_of_file ->
			Seq.Nil
	in
	aux 0
