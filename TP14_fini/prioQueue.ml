...
let rec repair_top_bottom compare tree =
	...
	| Node((Node(_, left_x, _) as left), x, (Node(_, right_x, _) as right)) ->
			...
			then
				...
				then Node(left, right_x, repair_top_bottom compare (change_root_unsafe right x))
				else Node(repair_top_bottom compare (change_root_unsafe left x), left_x, right)
			else
...
let change_root heap item =
	{ heap with tree = repair_top_bottom heap.compare (change_root_unsafe heap.tree item) }

let remove_top heap =
	let (tree_without_last, last) = remove_node heap.tree (path_of_size heap.size) in
	{
		size = heap.size - 1;
		tree = repair_top_bottom heap.compare (change_root_unsafe tree_without_last last);
